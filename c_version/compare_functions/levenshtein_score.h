...
double levenshtein_score(const char* str1, const char* str2)
{
    ...
    for (arr_idx1 = 1; arr_idx1 < arr_len1; arr_idx1++)
    {
        for (arr_idx2 = 1; arr_idx2 < arr_len2; arr_idx2++)
        {
            ...
            score_matrix[arr_idx1][arr_idx2] = MIN_OF_THREE(
                ...
                cost + score_matrix[arr_idx1 - 1][arr_idx2 - 1]
            );
        }
    }
    ...
    if (str_len1 > str_len2)
        return (str_len1 - score_matrix[str_len1][str_len2]) / (double)str_len1;
    else
        return (str_len2 - score_matrix[str_len1][str_len2]) / (double)str_len2;
}